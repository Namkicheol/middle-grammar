...
어휘 난이도 기준선은 `refs/NE-md변환/2022me_중1_L{N}_교과서.md` + `..._문법연습문제_기본AB.md` + `..._어법드릴문제.md` (중2는 통합 md). 해당 단원 자료에 등장하지 않거나 단원 학습 전 보지 못한 어휘는 "단원 외 어휘"로 간주한다.

---

## 의도적 설계 판별 기준 (점검 전 반드시 확인)

에이전트가 "오류"로 판단하기 전에 아래 세 가지 설계 패턴에 해당하는지 먼저 확인한다. 해당하면 **오류 또는 경고로 플래그하지 않는다.**

### A. 훼이크(의도적 대비 문제)

단원 목표 X에서 Y 문법이 정답인 문제를 넣어 구분력을 강화하는 전략. **아래 조건을 모두 충족하면 허용:**
1. answer-hint에 "← enjoy/keep 등은 동명사 사용", "← finish + ing" 등 대비 설명이 있음
2. 해당 문제가 섹션 전체의 30% 이내 (예: 10문제 섹션에서 3문제 이하)
3. MCQ 선택지에 to+V와 -ing가 모두 포함되어 학생이 구분해야 함

예시 (허용):
- L4(to부정사 단원) A섹션에서 `enjoy + watching`, `keep + making`이 정답인 문제 → 훼이크
- L2(동명사 단원) C섹션에서 현재진행형 vs 동명사 구분 10문제 → 훼이크

### B. 기초 워크시트 스캐폴딩

기초(basic) 워크시트 input 문제의 힌트에 문법 요소(조동사 `will`/`should`, to부정사 `to` 등)를 직접 제공하는 것은 **형태 훈련 스캐폴딩**이다. 기초 수준에서 선택 판단을 요구하지 않는 것은 정상 설계다. 오류·경고 대상 아님.

심화(hard) 워크시트에서 동일 힌트 형식이 반복되면 경고 대상.

### C. 명시적 다면 훈련

`sec-hdr`의 `.desc` 텍스트에 해당 서브섹션의 훈련 목적이 별도로 기술된 경우, 그 목적이 단원 메인 목표와 달라도 의도적 설계로 인정한다.

예시 (허용):
- L6 look+형용사 단원에서 sec-hdr가 "16–20: feel/smell/sound/taste **동사 수일치**"로 명시 → a16~a20 빈칸이 동사 자리인 것은 의도적 설계
...
해당 Lesson의 `refs/NE-md변환/` 자료를 어휘 수준 기준선으로 사용한다.
