...
description: middle-grammar 블로그 배포 워크플로우. 티스토리 기초영문법 카테고리 마크다운 글 작성(색깔 시스템/이미지), blog-deploy 파일 형식. 블로그 글 쓰거나 blog-deploy 파일 만들 때 호출.
---
...
블로그 글에서 **SVG 사용 금지**. 위계·흐름·비교는 다음으로 대체:
- **표** (위계·비교·분류)
...
- 색깔은 디폴트 적용, "빼줘" 요청 시에만 제외 (SVG는 항상 금지)
- **블로그 글은 `.md` 파일로 작성**
