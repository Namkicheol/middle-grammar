...
## Obsidian 운영 노트 연계 (2026-09-04)

- 보관함: `/Users/namgicheol/Documents/Obsidian/내 노트`. 공통 안내는 보관함의 `90 운영/자료 관리 원칙.md`, 이 레포의 운영 요약은 `10 교육/교육 프로젝트 목록.md` 안 `middle-grammar` 프로젝트 구역을 사용한다.
- 원본 콘텐츠·코드, 이 레포의 `AGENTS.md`·`docs/`, 검증·발행 기록이 기준이다. Obsidian은 탐색·개인 메모·요약용이며 원본이나 작업 지침을 대체하지 않는다.
- 관련 작업 시작 시 위 노트를 참고하되, 오래된 요약과 현재 원본이 다르면 현재 원본을 우선한다. 사용자 요청에 따른 콘텐츠·코드·운영 변경을 완료한 경우에만 해당 프로젝트 구역에 확인일, 검증된 변경·결과, 원본 링크, 남은 일을 짧게 날짜별로 누적한다. 단순 조회·진단·검토 요청에는 노트를 쓰지 않는다.
- 공용 노트의 개인 메모·다른 프로젝트 구역·기존 기록을 덮어쓰지 않는다. 로컬 기록, 실제 확인한 상태, 미확인 사항을 구분하며 발행·배포 완료를 로컬 기록만으로 단정하지 않는다.
- 임용 개념·답안·교재 내용을 복사하거나 새로 번역하지 않고 검증된 원본 링크 위주로 연결한다. 이 레포의 용어·저작권 규칙은 그대로 적용한다.
- 토큰·세션·개인정보·구독자 데이터·대용량 미디어를 보관함에 복사하지 않는다. 원본 파일을 이동하거나 원본 폴더를 보관함 안에 symlink로 노출하지 않는다.
- 보관함이 없거나 접근할 수 없어도 원본 작업은 계속하고 노트를 갱신하지 못했음을 보고한다. 대체 보관함 생성, Sync·Publish·플러그인 설정은 별도 요청 없이 하지 않는다.
- 이 연계 방침은 외부 발행·배포·commit·push 권한을 추가하지 않는다. 기존 승인·검증·Git 정책을 따른다.
