...
| ❌ to/for/of 전치사 선택 문제 → **심화에서만** | |

> **피드백 (2026-04-24):** 기초에서 to/for/of 전치사 구별 문제를 낸 결과 "너무 어렵다"는 지적 → 전치사 변환 유형은 심화 전용.
...
12. **블로그 배포 자료 — `blog-deploy/<unitNN-topic>.md` 파일로 저장** (신규 워크시트에만 해당)
    - **새 워크시트 파일이 처음 만들어졌을 때만** 아래 5종 세트를 `blog-deploy/` 폴더에 md 파일로 작성 후 커밋. 기존 파일 수정·리팩터링은 **작성하지 않는다** (사용자 피드백 2026-04-21).
    ...
    - ⑤ **agent 점검 결과** — answer-checker 실행 후 수정된 내용을 `### ⑤ agent 점검 결과 (날짜)` 섹션으로 md에 추가. 문제 수 변경 시 ①의 문제 수와 ④의 배지 문구도 함께 업데이트.
