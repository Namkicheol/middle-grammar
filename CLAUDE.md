...
function norm(s) {
  ...
  s = s.replace(/[''ʼ]/g, "'");  // 모바일 곡선 따옴표(U+2018/2019/02BC) → 직선
...
> **주의**: 곡선 따옴표 정규화 줄(`replace(/[''ʼ]/g,"'")`)은 반드시 축약형 치환보다 **앞에** 위치해야 한다. 모바일 키보드(iOS/Android)는 `doesn't`, `won't` 등 입력 시 U+2019(`'`) 곡선 따옴표를 삽입하므로, 이를 직선 `'`(U+0027)로 정규화하지 않으면 축약형→전체형 변환이 작동하지 않는다.
...
> **문장을 그대로 사용하는 것은 금지.** 예문의 구조·어휘·맥락을 참고하되, 인명·소재·수치 등을 바꾸어 반드시 변형해서 사용한다.
...
교과서·교재 예문을 참고할 때 아래 중 하나 이상을 반드시 변형한다.
...
✅ 변형: She (am / is) our coach.

❌ 원본: I kept waiting for one hour for my friend. (wait)
✅ 변형: She kept practicing for two hours for the show. (practice)

❌ 원본: When Paul came home, his sister was watching television.
✅ 변형: When Jake got home, his brother was playing video games.
```
...
13. **작업 완료 후 CLAUDE.md 업데이트 및 커밋·푸시** (2026-04-22~) — 새 규칙·발견된 버그 패턴·피드백 등 재사용 가치가 있는 내용은 작업 종료 전에 CLAUDE.md에 반영하고, 수정된 CLAUDE.md를 반드시 `git add → commit → push`까지 완료한다. 기존 파일 수정 작업에도 동일하게 적용.
...
- mcq 소재: 어법드릴 문장을 단어 교체해서 4지선다 구성
