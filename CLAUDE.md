...
**파일 위치**: `NE 능률 중2 교과서 자료/` (원본 .hwp — 직접 Read 불가. md 변환 후 사용)

```
NE 능률 중2 교과서 자료/
  2022me_중2_L{1~8}_문법연습문제/
    2022me_중2_L{N}_문법연습문제_기본AB.hwp
    2022me_중2_L{N}_문법연습문제_심화.hwp
```

> hwp 파일은 Claude가 직접 읽을 수 없다. 필요 시 사용자가 내용을 붙여넣거나 md로 변환해 제공.
> md 변환본이 없으면 Claude 자체 창작 예문 비율을 높여 진행한다.
...
12. **블로그 배포 자료 — `blog-deploy/<unitNN-topic>.md` 파일로 저장** (신규 워크시트에만 해당)
    ...
    - 파일 하나에 아래 항목을 **순서대로 모두** 작성한다 (상세 형식 → 아래 "블로그 배포 파일 형식" 섹션 참고):
      1. **제목** — `[중1 영문법] 또는 [중1 영문법 심화] + 문법명 + 라이브워크시트 + (N문제)`
      2. **태그** — 중1영어·중1영문법 계열 공통 + 문법 키워드 + 심화 여부
...
```
# [중1 영문법] 문법명 라이브워크시트 (N문제)

**태그**: 중1영어, 중1영문법, 키워드1, 키워드2, ...
