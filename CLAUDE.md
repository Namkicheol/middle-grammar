...
### 심화 워크시트 표준 구조 (past-be-hard 이후 모든 심화 공통)
...
- **passage 구조**: `.passage` div 안에 `.ptitle`, 인라인 `<span class="num">N</span>`, 하단에 질문 카드
- **JS**: `var TOTAL=25,SECTOTALS={a:15,b:10},secC={a:0,b:0},secW={a:0,b:0};`
- **억지 유형 금지**: 억지 유형은 더 유용한 유형(다른 MCQ, 더 나은 오류수정 등)으로 대체한다. 문제 수는 줄이지 않는다.
...
| 유형 | 설명 | 출처 예시 |
|------|------|-----------|
| 문맥 파악 MCQ | 빈칸에 알맞은 어형/단어 고르기 | Section A 첫 4문제 |
...
| 우리말 영작 | 한국어 해석 주고 괄호 단어로 영문 완성 | `_심화.md` C유형 |
| 짧은 지문 빈칸 | 짧은 지문(5~6문장) 안 빈칸 채우기 | Section B |
...
## 문제 내 한국어 해석 표시 규칙 (필수 · 2026-04-25)

### 핵심 판단 기준

**문장 구조(문법)가 문제인가, 문장 의미(해석)가 문제인가?**

- 문법 구조 판단 문제 (단어 배열·형태 고르기): 해석 없어도 풀 수 있음 → 상황에 따라 생략 가능
- **의미 이해가 선행돼야 풀 수 있는 문제**: 해석 **필수**

**해석이 필요한 대표 상황:**
- 어휘가 생소하거나 문장이 길어 의미 파악이 어려울 때
- because·when·that 등 접속사 의미 선택, 맥락 추론이 답을 결정할 때
- "올바른 표현 고르기" 유형에서 의미를 알아야 선택지 구분이 될 때
...
심화 워크시트(다크 네이비 테마)는 기본적으로 s0 문제에 ctx/q-kor 사용을 금지하지만, **의미 이해가 선행돼야 풀 수 있는 문제**는 예외로 `.q-kor` 또는 `q-sub` 안에 해석을 포함한다.

```html
<!-- 예: 어휘가 어렵거나 맥락이 필요한 심화 문제 -->
...
### 체크 포인트 (문제 작성 완료 후 자가 점검)

문제를 다 만들고 나서 반드시 아래를 확인한다:

> **"이 문제, 해석 없이 풀 수 있나?"**  
> → NO이면 → `.q-kor` 또는 `q-sub` 해석 추가  
> → YES이면 → 생략 가능

> 이 규칙은 신규 워크시트부터 적용. 기존 파일은 사용자 요청 시 업데이트.
...
4-1. **각 문제 완성 후 "해석 없이 풀 수 있나?" 점검** → NO이면 `.q-kor` 추가 (기초·심화 공통)
...
**억지 유형은 더 유용한 유형으로 대체한다.** "억지문제 만들지 말라" = 문제 수를 줄이라는 뜻이 아니라, 어색한 유형 대신 학습에 더 효과적인 유형으로 바꾸라는 뜻이다 (사용자 피드백 2026-04-25). 25~45문제 목표 수는 유지한다.
