...
상태: 설계 확정, 구현·운영 연결 전. 2026-09-06 사용자 지시에 따라 기존 Cloudflare Access 도입 계획을 대체한다.
...
현재 소스는 여전히 Access JWT를 요구한다. 이 문서를 추가하는 것만으로 운영 교사 API의 401이 해결되지는 않는다. 인증을 비활성화해 임시 공개하는 우회는 하지 않는다. 방탈출 및 다른 게임 작업은 유지하며, 교사 로그인 연결은 이 설계를 따른다.
